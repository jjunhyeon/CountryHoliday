...
public class GlobalHolidaySearchCondition extends PageRequestDto {
	...
	/** ISO 국가코드 (예: KR, US, JP) */
	@Schema(description = "ISO 2자리 국가코드", example = "KR")
	@Size(min = 2, max = 2, message = "국가코드는 ISO 2자리 코드값으로 입력해주세요.")
	private String countryCode;

	/** 휴일명 */
	@Schema(description = "공휴일명 키워드(부분 검색 가능)", example = " ")
	private String holidayName;

	/** 현지휴일명 */
	@Schema(description = "현지어 공휴일명 키워드", example = " ")
	private String localHolidayName;

	/** 조회 기준 연도 */
	@Schema(description = "조회 대상 연도 (2000~2099)", example = "2024")
	...
	/** 매년 날짜 동일 여부 (Y/N) */
	@Schema(description = "매년 날짜 동일 여부 (Y/N)", example = "N")
	private String isFixedDateYn;

	/** 국가 전체 적용 여부 (Y/N) */
	@Schema(description = "해당 국가 전체 공휴일 여부 (Y/N)", example = "Y")
	private String isNationalHolidayYn;

	/** 공휴일 유형 */
	@Schema(
	    description = "공휴일 유형 목록 (예: Public, Bank, School, Observance, Optional)",
	    example = "[\"Public\"]"
	)
